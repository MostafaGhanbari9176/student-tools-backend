...
class WorkSample
{
    ...
    public function edit($workSampleId, $subject, $description, $imgNum):bool
    {
        $sql = "UPDATE $this->tbName SET subject = ?, description = ?, img_num = ? WHERE work_sample_id = ?";
        $result = $this->con->prepare($sql);
        $result->bind_param('ssii', $subject, $description, $imgNum, $workSampleId);
        $data = $result->execute();
        $result->close();
        return $data;
    }
    ...
    {
        $sql = "SELECT t.seen_num , t.like_num , t.work_sample_id FROM $this->tbName t WHERE t.ability_id = ? AND NOT t.status = 4";
        $result = $this->con->prepare($sql);
        $result->bind_param('i', $abilityId);
        $result->execute();
    ...
    {
        $sql = "SELECT t.seen_num , t.like_num , t.work_sample_id FROM $this->tbName t WHERE t.ability_id = ? AND t.status = 1";
        $result = $this->con->prepare($sql);
        $result->bind_param('i', $abilityId);
        $result->execute();
        $data = $result->get_result();
    ...
    public function getSingle($workSampleId): mysqli_result
    {
        $sql = "SELECT t.img_id , t.seen_num , t.like_num , t.work_sample_id , t.subject , t.description , t.add_date FROM $this->tbName t WHERE t.ability_id = ? AND t.status = 1";
        $result = $this->con->prepare($sql);
        $result->bind_param('i', $abilityId);
        $result->execute();
        $data = $result->get_result();
